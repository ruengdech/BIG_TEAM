...
<% 
    ...
    cls.strSql = "SELECT        SYS02_MS_PRODUCT.PRODUCT_ID, SYS02_MS_PRODUCT.PRODUCT_BRAND_ID, SYS02_MS_PRODUCT.PRODUCT_NAME, SYS02_MS_PRODUCT.PRODUCT_STATUS, SYS02_MS_PRODUCT.PRODUCT_STAFF_ID,  " &
                    "                         SYS02_MS_BRAND.BRAND_NAME, SYS02_MS_STAFF.STAFF_NAME, SYS02_MS_PRODUCT.CWHEN " &
                    "FROM            SYS02_MS_PRODUCT INNER JOIN " &
                    "                         SYS02_MS_BRAND ON SYS02_MS_PRODUCT.PRODUCT_BRAND_ID = SYS02_MS_BRAND.BRAND_ID INNER JOIN " &
                    "                         SYS02_MS_STAFF ON SYS02_MS_PRODUCT.PRODUCT_STAFF_ID = SYS02_MS_STAFF.STAFF_ID " &
                    "ORDER BY SYS02_MS_BRAND.BRAND_NAME, SYS02_MS_STAFF.STAFF_NAME, SYS02_MS_PRODUCT.PRODUCT_NAME"
    cls.func_SetDataSet(cls.strSql, "JOB_DATA")

    cls.strSql = "SELECT  BRAND_ID, BRAND_NAME FROM SYS02_MS_BRAND WHERE BRAND_STATUS = 'ACTIVE'  ORDER BY BRAND_NAME "
    cls.func_SetDataSet(cls.strSql, "BRAND_DATA")

    'cls.strSql = "SELECT  STAFF_ID, STAFF_NAME FROM SYS02_MS_STAFF WHERE (STAFF_STATUS = 'ACTIVE') AND (STAFF_ROLE LIKE '%BRANDMANAGER%')  ORDER BY STAFF_NAME "
    cls.strSql = "SELECT  STAFF_ID,  STAFF_NAME + ' ' + STAFF_LNAME AS STAFF_NAME FROM SYS01_MS_STAFF WHERE  (STAFF_TYPE LIKE '%BRANDMANAGER%')  ORDER BY STAFF_NAME "
			...
			<div class="ui-grid-a ui-responsive">
				<div class="ui-block-a" style="width:65%;" >
                    ...
                    <table id="tb_list" class="table_list">
						<thead>
							<tr class="tr_main"><th>แบรนด์</th><th>BM</th><th style="width:50px">รหัส</th><th>ชื่อสินค้า</th><th> State </th><th> แก้ไข </th></tr>
                        </thead>
						<tbody>
							<% 
    If cls.ds.Tables("JOB_DATA").Rows.Count > 0 Then
        For i As Integer = 0 To cls.ds.Tables("JOB_DATA").Rows.Count - 1
            With cls.ds.Tables("JOB_DATA").Rows(i)
                ...
                Response.Write("<td>")
                Response.Write(.Item("BRAND_NAME").ToString())
                Response.Write("</td>")
                ...
                Response.Write("<td>")
                Response.Write(.Item("PRODUCT_ID").ToString())
                Response.Write("</td>")

                Response.Write("<td>")
                Response.Write(.Item("PRODUCT_NAME").ToString())
                Response.Write("</td>")

                Response.Write("<td>")
                Response.Write(.Item("PRODUCT_STATUS").ToString())
                Response.Write("</td>")

                Response.Write("<td class='center'>")
                Response.Write("<a href="""" onclick=""set_detail('" & .Item("PRODUCT_ID").ToString() & "')""><img src = '../RESOURCE/themes/images/set02/062-pencil.png' style='width:25px'  /> </a>")
                Response.Write("</td>")

                Response.Write("</tr>")
											%>
                                            <script>
                                                JOB_DATA.push(["<%response.Write(.Item("PRODUCT_ID").ToString)%>", "<%response.Write(.Item("PRODUCT_BRAND_ID").ToString)%>", "<%response.Write(.Item("PRODUCT_NAME").ToString)%>", "<%response.Write(.Item("PRODUCT_STATUS").ToString)%>", "<%response.Write(.Item("PRODUCT_STAFF_ID").ToString)%>", "<%response.Write(.Item("BRAND_NAME").ToString)%>", "<%response.Write(.Item("STAFF_NAME").ToString)%>", "<%response.Write(.Item("CWHEN").ToString)%>"]);
                                            </script>
											<%
                                                        End With
                                                    Next
				...
				<div class="ui-block-d" style="width:35%;" >
					...
					<div class="ui-grid-solo ui-responsive">
						...
						<div class="ui-block-a" style="padding-left:10px; padding-right:10px">
							<label for="sel_brand">แบรนด์:</label>
							<select id="sel_brand" name ="sel_type" data-mini="true">
						...
						<div class="ui-block-a" style="padding-left:10px; padding-right:10px">
							<label for="sel_staff">แบรนด์:</label>
							<select id="sel_staff" name ="sel_type" data-mini="true">
								<option value="" >เลือก BM</option>
								<%
						...
						<div class="ui-block-a" style="padding-left:10px; padding-right:10px">
							<label for="txt_name">สินค้า:</label>
		...
		<%--<div data-role="footer" data-position="fixed" style="background-color:#f1f1f3!important;vertical-align:middle;color:darkslategrey;border:none;">		
			<div class="ui-grid-d center">
				...
				<div class="ui-block-d" style="font-size:0.7em;" >สินค้า</div>
...
<script src="../JS/sys02.mng08.js"></script>
</html>
