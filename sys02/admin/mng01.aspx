...
<% 
    ...
    cls.strSql = "SELECT CUSTSTATE_ID, CUSTSTATE_NAME, CUSTSTATE_STATUS FROM SYS02_MS_CUSTOMER_STATE ORDER BY CUSTSTATE_NAME"
			...
			<div class="ui-grid-a ui-responsive">
				<div class="ui-block-a" style="width:65%;" >
                    ...
                    <table id="tb_list" class="table_list">
						<thead>
							<tr class="tr_main"><th style="width:50px">รหัส</th><th>ชื่อสถานะลูกค้า</th><th> State </th><th> แก้ไข </th></tr>
                        </thead>
						<tbody>
							<% 
    If cls.ds.Tables("JOB_DATA").Rows.Count > 0 Then
        For i As Integer = 0 To cls.ds.Tables("JOB_DATA").Rows.Count - 1
            With cls.ds.Tables("JOB_DATA").Rows(i)
                ...
                Response.Write("<td>")
                Response.Write(.Item("CUSTSTATE_ID").ToString())
                Response.Write("</td>")

                Response.Write("<td>")
                Response.Write(.Item("CUSTSTATE_NAME").ToString())
                Response.Write("</td>")

                Response.Write("<td>")
                Response.Write(.Item("CUSTSTATE_STATUS").ToString())
                Response.Write("</td>")

                Response.Write("<td class='center'>")
                Response.Write("<a href="""" onclick=""set_detail('" & .Item("CUSTSTATE_ID").ToString() & "')""><img src = '../RESOURCE/themes/images/set02/062-pencil.png' style='width:25px'  /> </a>")
                Response.Write("</td>")

                Response.Write("</tr>")
											%>
                                            <script>
												JOB_DATA.push(["<%response.Write(.Item("CUSTSTATE_ID").ToString)%>", "<%response.Write(.Item("CUSTSTATE_NAME").ToString)%>", "<%response.Write(.Item("CUSTSTATE_STATUS").ToString)%>"]);
                                            </script>
											<%
            End With
        Next
...
<script src="../JS/sys02.mng01.js"></script>
</html>
