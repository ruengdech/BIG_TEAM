...
<head runat="server">
    ...
    <link href="../sys02/RESOURCE/themes/ruengdech.css" rel="stylesheet" />
    <link href="../sys02/RESOURCE/themes/jquery.mobile.icons.min.css" rel="stylesheet" />
...
<% 
    ...
    If Session("USER_CATEGORY").ToString().IndexOf("DIRECTOR") > -1 Then
        ...
        cls.strSql = "SELECT SALE_ORDER_YEAR, SALE_ORDER_MONTH, SALE_HOSPITAL, BRAND_NAME, REVENUE, ORDERPROB_ID, ORDERPROB_NAME FROM SYS02_VIEW_CUSTOMER ORDER BY SALE_ORDER_YEAR, SALE_ORDER_MONTH, ORDERPROB_ID"
        cls.func_SetDataSet(cls.strSql, "JOB_DATA")

        cls.strSql = "SELECT SALE_HOSPITAL, BRAND_NAME, 0 AS [0], 0 AS [1], 0 AS [2], 0 AS [3], 0 AS [4], 0 AS [5], 0 AS [6], 0 AS [7], 0 AS [8], 0 AS [9], 0 AS [10], 0 AS [11], 0 AS [12] FROM SYS02_VIEW_CUSTOMER GROUP BY SALE_HOSPITAL, BRAND_NAME ORDER BY SALE_HOSPITAL, BRAND_NAME"
    ...
    Else
        ...
        cls.strSql = "SELECT SALE_ORDER_YEAR, SALE_ORDER_MONTH, SALE_HOSPITAL, BRAND_NAME, REVENUE, ORDERPROB_ID, ORDERPROB_NAME FROM SYS02_VIEW_CUSTOMER  WHERE (SALE_STAFF_AM_ID = '" & USER_STAFF_ID & "') OR  (SALE_STAFF_ID = '" & USER_STAFF_ID & "')  ORDER BY SALE_ORDER_YEAR, SALE_ORDER_MONTH, ORDERPROB_ID"
        cls.func_SetDataSet(cls.strSql, "JOB_DATA")
        ...
        cls.strSql = "SELECT SALE_HOSPITAL, BRAND_NAME, 0 AS [0], 0 AS [1], 0 AS [2], 0 AS [3], 0 AS [4], 0 AS [5], 0 AS [6], 0 AS [7], 0 AS [8], 0 AS [9], 0 AS [10], 0 AS [11], 0 AS [12] FROM SYS02_VIEW_CUSTOMER WHERE (SALE_STAFF_AM_ID = '" & USER_STAFF_ID & "') OR  (SALE_STAFF_ID = '" & USER_STAFF_ID & "') GROUP BY SALE_HOSPITAL, BRAND_NAME ORDER BY SALE_HOSPITAL, BRAND_NAME"
        cls.func_SetDataSet(cls.strSql, "STAFF_DATA") ' HOSPITAL_DATA
    End If
    '0   SALE_ORDER_YEAR
    '1   SALE_ORDER_MONTH
	...
	<div data-role="page" id="page_main">
		...
		<div data-role="content" class="ui-content" data-theme="c">
            ...
            <div class="ui-bar ui-bar-a center">
                <h3>Dashboard (Customer - Brand)</h3>
            </div>
...
<script src="../JS/sys02.view_customer01.js"></script>
    </html>
