        ...
        $.post(serverAJAXpath, {
            ...
            if (data.indexOf("SUCCESS") !== -1) {
                if (confirm("บันทึกสำเร็จ")) {
                    $.mobile.navigate("#page_main");
                    return true;
                    // Keep this Code but compare 2021-09-15
                    location.reload();
                } else {
                    $.mobile.navigate("#page_main");
                    return true;
...
function filter_data() {
    ...
    for (i = 0; i < JOB_DATA.length; i++) {
        ...
        if ((JOB_DATA[i][1].indexOf($("#s_date").val()) !== -1) && (JOB_DATA[i][2].toLowerCase().indexOf($("#s_hospital").val().toLowerCase()) !== -1) && (JOB_DATA[i][7].toLowerCase().indexOf($("#s_brand").val().toLowerCase()) !== -1) && ((JOB_DATA[i][6].toLowerCase() + JOB_DATA[i][5].toLowerCase()).indexOf($("#s_product").val().toLowerCase()) !== -1) && (JOB_DATA[i][8].indexOf($("#s_price").val()) !== -1) && (JOB_DATA[i][9].indexOf($("#s_qty").val()) !== -1) && (JOB_DATA[i][10].indexOf($("#s_total").val()) !== -1) && ((JOB_DATA[i][12].toLowerCase() + JOB_DATA[i][14].toLowerCase()).indexOf($("#s_status").val().toLowerCase()) !== -1) && (JOB_DATA[i][15].toLowerCase().indexOf($("#s_productin").val().toLowerCase()) !== -1) && (JOB_DATA[i][22].indexOf(s_year) !== -1) && (JOB_DATA[i][23].indexOf(s_month) !== -1) && (JOB_DATA[i][25].toLowerCase().indexOf($("#s_prob").val().toLowerCase()) !== -1) && (JOB_DATA[i][27].toLowerCase().indexOf($("#s_budget").val().toLowerCase()) !== -1) && (JOB_DATA[i][29].indexOf($("#s_sale").val()) !== -1) && (JOB_DATA[i][33].indexOf($("#s_am").val()) !== -1) && (JOB_DATA[i][31].indexOf($("#s_bm").val()) !== -1)) {
