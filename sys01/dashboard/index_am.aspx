<%@ Page Language="VB" AutoEventWireup="false" CodeFile="index_am.aspx.vb" Inherits="sys01_dashboard_index_am" %>
...
<body>
    <div data-role="page" data-theme="a">
		<div data-role="header" data-position="inline">
			...
			<h1>Saintmed: ระบบงานติดตั้ง (Dashboard AM)  </h1>
		</div>
    ...
    <form id="form1" runat="server">
        ...
        <table style="width: 100%;">
            <tr>
                <td class="auto-style1">&nbsp;Area Manager:&nbsp;
                    <asp:DropDownList ID="ddl_year" runat="server" AutoPostBack="True" data-native-menu="false" DataSourceID="DATASOURCE_AM_NAME" DataTextField="req_amname" DataValueField="req_amid">
                    </asp:DropDownList>
                    <asp:SqlDataSource ID="DATASOURCE_AM_NAME" runat="server" ConnectionString="<%$ ConnectionStrings:RUENGDECH_STMEDConnectionString %>" SelectCommand="SELECT req_amid, req_amname FROM SYS01_TS_REQUEST GROUP BY req_amid, req_amname ORDER BY req_amname"></asp:SqlDataSource>
                </td>
            </tr>
            <tr>
...
<HeaderStyle CssClass="header" BackColor="blue"></HeaderStyle>
                    ...
                    <asp:SqlDataSource ID="DATASOURCE_SALE" runat="server" ConnectionString="<%$ ConnectionStrings:RUENGDECH_STMEDConnectionString %>" SelectCommand="
SELECT staff_name AS SALE, count(*) AS Total, sum([ติดตั้งเครื่องมือแพทย์]) [ติดตั้งเครื่องมือแพทย์], sum([Demo]) [Demo], sum([ย้ายเครื่องมือแพทย์]) [ย้ายเครื่องมือแพทย์], sum([แสดงสินค้า]) [แสดงสินค้า], sum([สำรองเครื่อง]) [สำรองเครื่อง], sum([ตรวจเช็คระบบ]) [ตรวจเช็คระบบ], sum([Preventive Maintainance]) [PM], sum([อื่นๆ]) [อื่นๆ] ,CONCAT('index_sale.aspx?sale=' , staff_id , '&amp;year=', @Pyear) as LINK


FROM SYS01_TS_REQUEST PIVOT (count(engineer_job_detail) FOR req_jobtype1 IN ([ติดตั้งเครื่องมือแพทย์], [Demo], [ย้ายเครื่องมือแพทย์], [แสดงสินค้า], [สำรองเครื่อง], [ตรวจเช็คระบบ], [Preventive Maintainance], [อื่นๆ])) AS MAIN_DATA WHERE req_status &lt;&gt; 'INACTIVE' AND year(req_installdate) = @Pyear and req_amid = @AM_ID GROUP BY staff_name,CONCAT('index_sale.aspx?sale=' , staff_id , '&amp;year=', @Pyear)">
                        <SelectParameters>
...
WHERE req_status &lt;&gt; 'INACTIVE' and year(req_installdate) = @Pyear  and req_amid = @AM_ID 
group by req_status
">
                        <SelectParameters>
...
group by CASE WHEN DATEDIFF(day, req_date, req_installdate) &lt;=3 THEN '01 งานด่วนมาก' ELSE CASE WHEN DATEDIFF(day, req_date, req_installdate) &lt;=6 THEN '02 งานด่วน' ELSE '03 งานปรกติ' END  END ">
            ...
            <tr>
                ...
                <td>
                    ...
                    <asp:SqlDataSource ID="DATASOURCE_RAW" runat="server" ConnectionString="<%$ ConnectionStrings:RUENGDECH_STMEDConnectionString %>" SelectCommand="SELECT [uid] JOB_ID, [staff_name] SALE , [req_installlocation] Location, [req_installdate] Date, [req_jobtype1] Type, [req_job1] Topic, [req_staffname1] Engineer, [req_jobdatefinish] Finished, [req_status] Status FROM [SYS01_TS_REQUEST] 
